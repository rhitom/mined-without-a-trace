...
- **Prologue redesigned (Option 1 — comic strip grid):** Replaced broken clip-path shoe animation with a 2×2 CSS grid. `BG = "#EDE8DC"` matches illustration backgrounds. Panels stagger in with opacity+scale (80ms + 160ms per panel). Click any panel → ink-wipe → full-screen zoomed mode. Zoomed mode advances on click with ink-wipe; last panel → `/select`. Pill-style dot nav + hint text. Panel numbers (01–04) top-left of each cell.
